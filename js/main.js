/**
 * Created by Alexander on 5/7/2017.
 */
...
function getTask() {
    ...
    $.get('https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/tasks/' + task_id + '?apiKey=' + api_key, function( data ) {
...
function getCategoryOptions(){
    $.get('https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/categories?apiKey=' + api_key, function( data ){
        var output;
        $.each( data, function( key, data ){
            console.log( data );
...
function getCategories(){
    $.get('https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/categories?apiKey=' + api_key, function(data){
        var output = '<ul class="list-group">';
        $.each( data, function( key, data ) {
           output += '<li class="list-group-item category">' + data.category_name + '<div class="pull-right"><a class="btn btn-primary btn-edit-category" data-category-id="'+ data._id.$oid +'">Edit</a>  <a class="btn btn-danger btn-delete-category" data-category-id="'+ data._id.$oid +'">Delete</a></div>' +
                   '</li>';
        });
        output += '</ul>';
...
function editTask() {
    var task_id = sessionStorage.getItem( 'currentTaskId' );
    console.log( 'The id is: ' + task_id );
    var task_name = $('#task_name').val();
    var category = $('#category').val();
    var due_date = $('#due_date').val();
    var is_urgent = $('#is_urgent').val();

    $.ajax({ url: 'https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/tasks/' + task_id + '?apiKey=' + api_key,
            data: JSON.stringify({
                "task_name": task_name,
                "category": category,
                "due_date": due_date,
...
function deleteTask() {
    var task_id = $( this ).data( 'task-id' );
    $.ajax({
        url: 'https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/tasks/' + task_id + '?apiKey=' + api_key,
        type: "DELETE",
        async: true,
        timeout: 300000,
        success: function( data ){
...
function addTask(){
    ...
    var is_urgent = $('#is_urgent').val();
    $.ajax({
        url: 'https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/tasks?apiKey=' + api_key,
        data: JSON.stringify({
            "task_name" : task_name,
...
function addCategory(){
    ...
    $.ajax({
        url: 'https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/categories?apiKey=' + api_key,
...
function getCategory() {
    ...
    $.get('https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/categories/' + category_id + '?apiKey=' + api_key,
        function( data ){
...
function editCategory(){
    ...
    var category_name = $('#category_name').val();
    $.ajax({
        url: 'https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/categories/' + category_id + '?apiKey=' + api_key,
        data: JSON.stringify({"category_name" : category_name}),
        type: "PUT", //"PUT" is used for updating
        contentType: "application/json",
        success: function(data){
            window.location.href='categories.html';
        },
...
function deleteCategory(){
    ...
    $.ajax({
        url: 'https://api.mlab.com/api/1/databases/learn_mongo_task_manager/collections/categories/' + category_id + '?apiKey=' + api_key,
        type: "DELETE", //"DELETE" is used for deleting objects
        async: true,
        timeout: 300000,
        success: function(data){
            window.location.href='categories.html';
